...
public class JobListAdapter extends RecyclerView.Adapter<JobViewHolder>{
    ...
    public void onBindViewHolder(final JobViewHolder holder, int position) {
        ...
        if(MainActivity.isJobSaved(MainActivity.context,list.get(position).getPost(),list.get(position).getInstitute(),list.get(position).getPlace(),list.get(position).getURL())){
            savedIcon.setImageResource(R.drawable.ic_saved_icon);
        }


        if(MainActivity.isJobLoved(MainActivity.context,list.get(position).getPost(),list.get(position).getInstitute(),list.get(position).getPlace(),list.get(position).getURL())){
